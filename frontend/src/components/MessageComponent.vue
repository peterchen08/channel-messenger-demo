		...
		<fieldset v-for="item in requests" :key="item.id">
			...
			<div class="content">
				<div class="request">
					...
					<q-btn color="secondary" icon="send" label="Execute" class="q-ml-md shadow-11" @click="callAction(item)" />
				</div>
				<div v-if="item.fields" class="parameters">
					<h4>Parameters:</h4>
					<div class="field" v-for="field in item.fields" :key="field.field">
						<q-input v-if="field.type == 'text'" clearable clear-icon="close" filled color="purple-12" v-model="fields[field.model]" :label="field.label" :type="field.type" />
						<q-input v-else clearable clear-icon="close" filled color="purple-12" v-model.number="fields[field.model]" :label="field.label" :type="field.type" />
						<!-- <input
								:type="field.type"
				...
				<div class="response" v-if="item.status">
					<h4>
						Response:
						<q-badge :color="item.status < 400 ? 'green' : 'red'">{{ item.status }}
						</q-badge>

						<q-badge color="black" class="q-ml-sm">{{ humanize(item.duration) }}
						</q-badge>
					</h4>
...
let requests = reactive([
	{
		...
		afterResponse: (response: any) =>
			...
			// @ts-ignore
			!this.fields.channelID && (this.fields.channelID = response.rows[0]._id),
	},
	{
		id: 'create',
		action: 'v1.messages.create',
		rest: '/api/v1/messages/',
		method: 'POST',
		fields: [
			{
		...
		afterResponse: (response: any) =>
			// eslint-disable-next-line @typescript-eslint/ban-ts-comment
			// @ts-ignore
			(this.fields.channelID = response._id),
	},
	{
		id: 'get',
		action: 'v1.messages.get',
		rest: '/api/v1/messages/:id',
		method: 'GET',
		fields: [{ field: 'id', label: 'ID', type: 'text', paramType: 'url', model: 'messageID' }],
		response: null,
		status: null,
		duration: null,
	},
	{
		id: 'update',
		action: 'v1.messages.update',
		rest: '/api/v1/messages/:id',
		method: 'PUT',
		fields: [
			{ field: 'id', label: 'ID', type: 'text', paramType: 'url', model: 'messageID' },
			{
				field: 'title',
				label: 'Message Title',
				type: 'text',
			...
			requests = reactive([
				{
					...
					afterResponse: (response: any) =>
						// eslint-disable-next-line @typescript-eslint/ban-ts-comment
						// @ts-ignore
						!this.fields.messageID && (this.fields.messageID = response.rows[0]._id),
				},
				{
					id: 'create',
					action: 'v1.messages.create',
